...
export PGPASSWORD=facia-tool

docker-compose -f "${SCRIPT_PATH}/../docker-compose.yml" up -d

until pg_isready -h localhost -p 5432 -d facia-tool -U facia-tool > /dev/null
do
...
TABLE_EXISTS=$(psql -h localhost -p 5432 -U facia-tool -d facia-tool -t -c "${EXISTS_QUERY}" | xargs)

if [ "$TABLE_EXISTS" = "f" ]
then
    echo "    > Database has not been setup!"
    echo "    > Setting up database..."
    psql -h localhost -p 5432 -U facia-tool -d facia-tool -t -f "${DB_SETUP}" > /dev/null
    echo "    > Setup exit code = $?"

    INSERT_DB_HASH_QUERY="INSERT INTO manifest (key, value) VALUES ('setup_hash', '${SETUP_HASH}')"
    psql -h localhost -p 5432 -U facia-tool -d facia-tool -t -c "${INSERT_DB_HASH_QUERY}" > /dev/null
else
    DB_SETUP_HASH_QUERY="SELECT value FROM manifest WHERE key = 'setup_hash'"
    DB_SETUP_HASH=$(psql -h localhost -p 5432 -U facia-tool -d facia-tool -t -c "${DB_SETUP_HASH_QUERY}" | xargs)
...
#echo ""
#echo 'Starting facia-tool server!'
