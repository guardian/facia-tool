...
interface DispatchProps {
	updateCollectionConfig: (collection: CollectionConfig) => Promise<void>;
...
const CollectionMetadataForm = ({
	...
	const buildInitialState = (): FormState => {
		...
		return {
			type: currentType,
			displayName: collectionConfig?.displayName ?? displayName,
			...
			uneditable: collectionConfig?.uneditable ?? false,
			displayEditWarning:
				collectionConfig?.frontsToolSettings?.displayEditWarning ?? false,
			suppressImages: collectionConfig?.displayHints?.suppressImages ?? false,
			maxItemsToDisplay: String(
				collectionConfig?.displayHints?.maxItemsToDisplay ?? '',
			),
			metadataTags: (collectionConfig?.metadata ?? []).map((m) => m.type),
