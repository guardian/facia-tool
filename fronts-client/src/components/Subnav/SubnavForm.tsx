...
interface SubnavFormProps {
	// When provided the form is in edit mode, seeded from this subnav.
	initialSubnav?: CustomSubnav;
	onSaveDraft: (subnav: CustomSubnav) => Promise<void>;
	onPublish: (id: string) => Promise<void>;
	saving: boolean;
	// Edit-mode only: current server status and lifecycle actions.
	hasLive?: boolean;
	hasDraft?: boolean;
...
const SubnavForm = ({
	...
	const [links, setLinks] = useState<LinkRow[]>(
		initialSubnav && initialSubnav.links.length > 0
			? initialSubnav.links.map((link) => ({ ...link, id: v4() }))
			: [emptyLink()],
	);
	const [pages, setPages] = useState<TargetedPage[]>(
		initialSubnav && initialSubnav.pages.length > 0
			? initialSubnav.pages.map((page) => ({ ...page }))
			: [emptyPage()],
	);
	...
	// Whether a publishable draft exists on the server (or was just saved).
	const [draftSaved, setDraftSaved] = useState(
		isEditMode ? initialHasDraft : false,
	);
	...
	const canPublish = draftSaved && !isDirty && requiredComplete && !saving;
					...
					<CreateFormMain>
						...
						>
							...
							<CreateFormActions>
								...
								{isEditMode && hasLive && draftSaved && onDiscard && (
									<Button
										variant="secondary"
										size="sm"
										onPress={() => onDiscard(subnavId.current)}
										isDisabled={actionPending || saving}
									>
										Discard changes
									</Button>
								)}
								...
								{isEditMode && hasLive && onUnpublish && (
									<Button
										variant="secondary"
										size="sm"
										onPress={() => onUnpublish(subnavId.current)}
										isDisabled={actionPending || saving}
									>
										Take down
									</Button>
								...
								{isEditMode && onDelete && (
									<Button
										variant="tertiary"
										size="sm"
										cssOverrides={deleteButtonStyles}
										onPress={() => onDelete(subnavId.current)}
										isDisabled={actionPending || saving}
									>
										Delete
									</Button>
