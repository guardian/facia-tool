...
class AppComponents(context: Context, val config: ApplicationConfiguration)
  ...
  val mapiFrontEventQueue = PressQueue(config.faciatool.mapiFrontEventQueue, config.aws.mapiAccountCredentials)
  val faciaPressQueue = PressQueue(config.faciatool.frontPressToolQueue, config.aws.frontendAccountCredentials)
  val faciaPress = new FaciaPress(faciaPressQueue, mapiFrontEventQueue, configAgent)
