...
trait PanDomainAuthActions extends AuthActions with PanDomainAuth with Results {
  ...
  private def userInGroups(authedUser: AuthenticatedUser): Boolean = groupChecker.exists{ checker =>
    checker.checkGroups(authedUser, config.pandomain.userGroups).fold(err => false, identity)
  }
  ...
  override def invalidUserMessage(claimedAuth: AuthenticatedUser): String = {
    if( (claimedAuth.user.emailDomain == "guardian.co.uk") && !claimedAuth.multiFactor) {
      ...
      Please contact the Helpdesk by emailing 34444@theguardian.com or calling 34444 and request access to Composer CMS tools."""
    } else if (!userInGroups(claimedAuth)) {
      s"""${claimedAuth.user.email} is not valid for use with the Fronts Tool as you need to be granted access to the editorial tools.
      Please contact the Helpdesk by emailing 34444@theguardian.com or calling 34444 and request access to Composer CMS tools."""
    } else {
      s"""${claimedAuth.user.email} is not valid for use with the Fronts Tool. You need to use your Guardian Google account to login.
      Please sign in with your Guardian Google account first, then retry logging in."""
    }
  }
