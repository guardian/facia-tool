...
class EditionsDBTest extends FreeSpec with Matchers with EditionsDBService with OptionValues {
  ...
  "The editions DB" - {
    ...
    "should list the empty issue" taggedAs UsesDatabase in {
      val issues = editionsDB.listIssues("daily-edition", LocalDate.of(2019, 9, 29), LocalDate.of(2019, 10, 3))
      issues.length shouldBe 1
      issues.head.createdEmail shouldBe "billy.bragg@justice.example.com"
    }
