...
class S3client (conf: SwitchboardConfiguration) {
  ...
  def get(): Option[Map[String, Boolean]] = {
    ...
    Try[S3Object](client.getObject(request)) match {
      case Success(result) => {
        val resultAsString: String = Source.fromInputStream(result.getObjectContent).mkString
        result.close()
        Try(Json.parse(resultAsString)) match {
          case Success(json) => json.validate[Map[String, Boolean]] match {
            case JsSuccess(m, _) => {
          ...
          case Failure(_) => {
            Logger.error("invalid json format at %s - %s : %s" format(bucket, objectKey, resultAsString))
            None
          }
        }
      }
