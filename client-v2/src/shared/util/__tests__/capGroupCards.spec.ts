...
const toFragmentArray = (groups: Group[]) => groups.map(({ cards }) => cards);
...
describe('capGroupCards', () => {
  it('removes cards in an array of groups after a given maximum', () => {
    ...
    const post = toFragmentArray(capGroupCards(groups, 5));
  ...
  it('keeps the same array reference if the cards remain untouched', () => {
    ...
    const pre = toFragmentArray(groups);
    const post = toFragmentArray(capGroupCards(groups, 10));
