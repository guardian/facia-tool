...
dependencyOverrides ++= Seq(
	...
	// Pinned to resolve transitive dependencies between Scanamo and AWS SDK
	...
	"io.netty" % "netty-transport-classes-epoll" % "4.1.125.Final"
)

val UsesDatabaseTest =
	config("database-int") extend Test
		...
		.settings(
			...
			buildInfoKeys := {
				Seq[BuildInfoKey](
					...
					BuildInfoKey.constant(
						"gitCommitId",
						env("GITHUB_SHA").getOrElse("unknown")
					)
				)
			}
		)
